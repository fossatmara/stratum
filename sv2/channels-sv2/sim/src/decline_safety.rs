...
//! criterion (METRIC_DERIVATION.md §9.2), so it earns its own dedicated guard.
...
///
/// Provenance: `docs/METRIC_DERIVATION.md` §9.2 and `docs/SLOW_DECLINE_TEST.md`.
...
mod tests {
    ...
    /// selection criterion (METRIC_DERIVATION.md §9.2); the grid baseline does
    ...
    fn champion_clears_decline_gate() {
        ...
        assert!(
            ...
            "shipped champion's worst settled over-difficulty {:+.2}% exceeds the \
             ...
             see decline_safety.rs / METRIC_DERIVATION.md §9.2.)",
            worst,
