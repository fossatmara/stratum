...
def content_match(docroot, ref):
    """Re-read the pinned lines and check the required content substring is there.
    Returns (ok, detail). Content-match, not existence: the pin must SAY the thing."""
    doc = ref.get("doc")
    must = ref.get("content_must_match")
    if doc in (None, "this-section") or must is None:
        # in-section or contentless source claims can't be line-pinned here; the
        # honest result is 'unverifiable by this gate', surfaced, not silently passed.
        return None, f"no external pin to content-match (doc={doc})"
    path = os.path.join(docroot, doc)
    if not os.path.exists(path):
        return False, f"pinned doc not found: {path}"
    text = open(path, encoding="utf-8", errors="replace").read()
    needle = must.replace("\n>   ", " ").replace("\n", " ")
    hay = re.sub(r"\s+", " ", text)
    return (needle in hay or must in text), f"content_must_match {'FOUND' if (needle in hay or must in text) else 'MISSING'}: {must[:50]!r}"
