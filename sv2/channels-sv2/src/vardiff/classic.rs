...
impl Vardiff for VardiffState {
    ...
    fn try_vardiff(
        ...
        let dt = match now.checked_sub(self.timestamp_of_last_update) {
            Some(dt) => dt,
            None => {
                ...
                // window to `now` and skip just this round: the `dt <= 15` guard below
                // returns before `reset_counter()` runs, so without re-anchoring here every
                // later round would measure against the same stale future timestamp and
                // vardiff would stall for the whole length of the backwards step.
                debug!(
                    target: "vardiff",
        ...
        // It is the divisor for the deviation below and the base `compute_new_hashrate`
        // retargets from, so `0.0`/`NaN` produce `inf`/`NaN` deviations that disable the
        // fire comparison outright. The output is already clamped to the floor below, so
        // clamp the baseline to the same floor. `is_finite` is still needed: `inf` survives
        // a plain `max()` and makes the deviation `inf / inf == NaN`.
