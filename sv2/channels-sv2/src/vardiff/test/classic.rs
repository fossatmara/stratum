...
/// Guard tests carried over verbatim from upstream's two hardening commits — the
/// unusable-prior-hashrate fix (`project-loupe/audit-stratum#40`, a vardiff bypass enabling
/// low-difficulty share flooding) and the backwards-clock `checked_sub` fix. They are kept
/// against upstream's own consts and harness rather than restated in this file's style, so
/// the security properties are asserted exactly as upstream wrote them. All four must keep
/// passing against the champion: it changes which algorithm decides a retarget, not whether
/// a miner-supplied `nominal_hash_rate` can be `0.0`/`NaN`.
mod upstream_guards {
    use super::super::{
        simulate_shares_and_wait, test_backwards_clock_step_reanchors_window,
        TEST_MIN_ALLOWED_HASHRATE, TEST_SHARES_PER_MINUTE,
    };
    use crate::vardiff::classic::VardiffState;
    use crate::{target::hash_rate_to_target, vardiff::error::VardiffError, vardiff::Vardiff};
